...
def collapse(result, temperature):
    ...
    for q in range(lgprocess.NUM_FILTERS):
        dist = np.nan_to_num(np.exp(np.log(sample[0, 0, q]) / temperature), 0)
        m = np.sum(dist)
        if m > 0:
            dist /= m
        else:
...
def sample(epoch, header, headerLen, num_chars, temperature):
    ...
    if header != "":
        ...
        for i in range(headerLen):
            s = np.zeros((1, 1, lgprocess.NUM_FILTERS * 2, lgprocess.QUANT_MAX), np.float32)
            for q in range(lgprocess.NUM_FILTERS * 2):
                s[0, 0, q, int(seeddata[i, q // 2, q % 2])] = 1
            sampled.append(s)
    ...
    for i in range(num_chars):
        ...
        sample = collapse(result, temperature)
...
if __name__ == '__main__':
    ...
    for i in range(len(output)):
        s = output[i]
        for q in range(lgprocess.NUM_FILTERS):
            packedOut[i, q, 0] = np.argmax(s[0, 0, q * 2 + 0])
            packedOut[i, q, 1] = np.argmax(s[0, 0, q * 2 + 1])
