...
def read_batches(T):
    ...
    for start in range(0, batch_chars - SEQ_LEN, SEQ_LEN):
        ...
        Y = np.zeros((BATCH_SIZE, SEQ_LEN, lgprocess.NUM_FILTERS * 2, lgprocess.QUANT_MAX), np.float32)
        for batch_idx in range(BATCH_SIZE):
            for i in range(SEQ_LEN):
                for q in range(lgprocess.NUM_FILTERS * 2):
                    X[batch_idx, i, q, int(T[batch_chars * batch_idx + start + i, q // 2, q % 2])] = 1
                    Y[batch_idx, i, q, int(T[batch_chars * batch_idx + start + i + 1, q // 2, q % 2])] = 1
