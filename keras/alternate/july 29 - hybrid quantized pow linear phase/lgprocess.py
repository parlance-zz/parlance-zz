...
def ComplexQuantize(filter, quantMax, powOffset, powScale):  
    ...
    for i in range(len(filter)):
        c = filter[i]
        phase = np.floor((np.angle(c) + np.pi) / 2 / np.pi * quantMax + 0.5) % QUANT_MAX
        #phase = np.angle(c)
...
if __name__ == '__main__':
    ...
    for i in range(0, numOutputSamples - FFT_LEN, CHUNK_STEP):
        ...
        if args.outputraw != '':
            ...
            reconstructed = InverseFilterSet(FFT_LEN, lgFilters, lgResponse, lgFilterBounds)
