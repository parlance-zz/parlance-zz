...
Function run_frame()
	...
	PokeByte ram,$2002,(PeekByte(ram,$2002) And $3F) ;we;re no longer in vblank
													 ;kills the sprite-0 hit bit too
	...
	For scan_line=0 To 240-1
		...
		If PeekByte(ram,$2001) And $8
			...
			For pixel_x=0 To 256-1
	
				If scan_line = PeekByte(spr_ram,0) And pixel_x = PeekByte(spr_ram,3) Then PokeByte ram, $2002, (PeekByte(ram,$2002) Or $40) ;sprite 0 hit
				
				If (pixel_x) Mod 16 = 0 ;this ratio gives 144 cycles per scanline
				...
				pixel_x = pixel_x + scroll_x
				scan_line = scan_line + scroll_y
					...
					For px=0 To 8-1
						...
						If h_flip
							...
							If v_flip
								...
								If sprite_size
									...
									color_index=((PeekByte(vram,opposite_address+7-py) And ($1 Shl px)) Or ((PeekByte(vram,opposite_address+7-py+8) And ($1 Shl px)) Shl 1 )) Shr (px)
								
								Else
								
									color_index=((PeekByte(vram,base_address+7-py) And ($1 Shl px)) Or ((PeekByte(vram,base_address+7-py+8) And ($1 Shl px)) Shl 1 )) Shr (px)
						...
						Else
							...
							If v_flip
								...
								color_index=((PeekByte(vram,base_address+7-py) And ($80 Shr px)) Or ((PeekByte(vram,base_address+7-py+8) And ($80 Shr px)) Shl 1 )) Shr (7-px)
						...
						For px=0 To 8-1
							...
							If h_flip
								...
								If v_flip
									...
									color_index=((PeekByte(vram,base_address+15-py) And ($1 Shl px)) Or ((PeekByte(vram,base_address+15-py+8) And ($1 Shl px)) Shl 1 )) Shr (px)
																	
								Else
								
									color_index=((PeekByte(vram,opposite_address+py) And ($1 Shl px)) Or ((PeekByte(vram,opposite_address+py+8) And ($1 Shl px)) Shl 1 )) Shr (px)
							...
							Else
								...
								If v_flip
									...
									color_index=((PeekByte(vram,base_address+15-py) And ($80 Shr px)) Or ((PeekByte(vram,base_address+15-py+8) And ($80 Shr px)) Shl 1 )) Shr (7-px)
